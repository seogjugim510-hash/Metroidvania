...
namespace RopeWrapper
{
    ...
    public class RopeShootController : MonoBehaviour
    {
        ...
        private void ShootTheRope()
        {
            ...
            GetComponent<Rigidbody2D>().linearVelocity = ropeShootVelocity * shootVector.normalized;
        ...
        private void OnTriggerEnter2D(Collider2D collision)
        {
            if (collision.gameObject.name != player.name) //Rope cannot anchor on player itself
            {
                ...
                GetComponent<Rigidbody2D>().linearVelocity = Vector2.zero;                //Anchor the rope by stopping the rope movement. 
